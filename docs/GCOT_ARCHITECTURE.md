...
- Makes allow/block decisions by returning the configured block-page IP for blacklisted domains
...
- Contains blacklist entries mapped to the NTC block-page IP
...
4. **CoreDNS** → Checks policies
    - Is domain in blacklist? → Return the block-page IP
...
DNS-only enforcement cannot transparently redirect HTTPS requests for arbitrary blocked domains without a trusted endpoint mechanism. HTTPS clients validate the original hostname before any HTTP redirect can be sent, so the no-root-CA deployment uses TCP/443 fail-fast behavior to avoid browser timeouts rather than attempting an HTTPS redirect.

...
export function AddDomainDialog({ type, onSubmit, triggerLabel, submitLabel }: DomainFormProps) {
  ...
  const handleSubmit = () => {
    const data = {
      domain,
      ...(type === 'blacklist' && { threatLevel, sources }),
      ...(type === 'whitelist' && { reason }),
    };
    onSubmit?.(data);
    setOpen(false);
    setDomain('');
        ...
        <div className="space-y-4 py-4">
          ...
          {type === 'whitelist' && (
            <div className="space-y-2">
              ...
              <Textarea
                id="reason"
                placeholder="Explain why this domain should be whitelisted..."
                value={reason}
...
export function BulkImportDialog({ type, onSubmit, triggerLabel, submitLabel }: DomainFormProps) {
  const [open, setOpen] = useState(false);
